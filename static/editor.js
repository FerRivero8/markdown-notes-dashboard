document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ Verifica si hay token y redirige si no
    ...
    button.addEventListener('click', async () => {
      ...
      if (type) {
        ...
        try {
          ...
          showToast('Error de red o token inválido.', 'error');
        }
      }
    });
  });
  ...
  function applyMarkdown(type) {
    ...
    if (selected.length > 0) {
      switch (type) {
        ...
        case 'link': insertText = `[${selected || 'enlace'}](https://)`; break;
        case 'image': insertText = `![${selected || 'descripción'}](https://)`; break;
      }
